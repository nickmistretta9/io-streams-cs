...
namespace SoccerStats
{
    class MainClass
    {
        public static void Main(string[] args)
        {
            ...
            foreach(var player in topTenPlayers) {
                ...
                foreach(var result in newsResults) {
                    Console.WriteLine(string.Format("Sentiment Score:D {0:P}, Date: {1:f}, Headline: {2}, Summary: {3} \r\n", result.SentimentScore, result.DatePublished, result.Headline, result.Summary));
                    Console.ReadKey();
                }
            }
		...
		public static List<NewsResult> GetNewsForPlayer(string playerName)
		{
			...
			var webClient = new WebClient();
            webClient.Headers.Add("Ocp-Apim-Subscription-Key", "68139ab542884e73af903d09645b68c0");
            byte[] searchResults = webClient.DownloadData(string.Format("https://api.cognitive.microsoft.com/bing/v5.0/news?q={0}&mkt=en-us", playerName));
            var serializer = new JsonSerializer();
        ...
        public static SentimentResponse GetSentimentResponse(List<NewsResult> newsResults) {
            ...
            byte[] response = webClient.UploadData("https:D//westus.api.cognitive.microsoft.com/text/analytics/v2.0/sentiment", requestBytes);
